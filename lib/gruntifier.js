...
var Gruntifier = function (grunt, done, bust) {
	...
	_Gruntifier.prototype = {
		...
		setupTests : function () {
			...
			if (!_quiet && extraTests.length) {
				grunt.log.subhead("Enabled Extras");
				grunt.log.ok(extraTests.join(", ").grey);
			}
			...
			for (key in config.extensibility) {
				...
				if (value) {
					extensibilityTests.push(key);
					allTests.push(key);
				}
			}

			if (!_quiet && extensibilityTests.length) {
				grunt.log.subhead("Enabled Extensibility Options");
				grunt.log.ok(extensibilityTests.join(", ").grey);
			}
		...
		parseData : function (file, data) {
			...
			if (!_quiet && matchedTests && matchedTests.length) {
				...
				grunt.log.ok(matchedTests.join(", ").grey);
			}
		},
				...
				for (i = 0, j = pathname.length; i < j; i++) {
					...
					if (!bust && fs.existsSync(cachePath)) {
						...
						if (lastModified < timeBuffer) {
							...
							if (!_quiet) {
								grunt.log.writeln("cache ".green + basename.grey);
							}
		...
		makeRequests : function (tests) {
			...
			promise.all(
				...
				promise.when(this.xhr(communityRequests)).then(function (community) {
					...
					if (!_quiet) {
						grunt.log.writeln();
						grunt.log.ok("Generating a custom Modernizr build".grey);
					}
		...
		finalize : function (data, tests, customTests) {
			...
			var build = Modulizr.ize(data, this.getRequests(tests)),
				...
				sourceMapName, buildOptions, code,
				errorText, i, j;
			...
			if (config.uglify) {
				...
				build = uglifier.minify(build, buildOptions);
				code = build.code;
			} else if (!_quiet) {
				grunt.log.ok("Skipping uglify".grey);
			}

			// Prefix with Modernizr licence
			code = this.addPrefix(code, tests, customTests);
			...
			// All set.
			if (!_quiet) {
				grunt.log.subhead("Wrote minified file to " + config.outputFile);
			}
