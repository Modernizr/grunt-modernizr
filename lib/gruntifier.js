	...
	_Gruntifier.prototype = {
		...
		setupTests : function () {
			var allTests = config.tests, key, value,
				extraTitle, extensibilityTitle, communityTitle;
			...
			for (key in config.community) {
				value = config.community[key];

				if (value) {
					if (!_quiet && !communityTitle) {
						grunt.log.subhead("Enabled Community");
						communityTitle = true;
					}
