...
var Gruntifier = function (grunt, done, bust) {
	...
	_Gruntifier.prototype = {
		init : function () {
			...
			this.fetchMetadata().then(function (metadata) {

				var tests = this.crawlFilesForTests(metadata);
				var config = this.writeConfig(tests);

				this.buildModernizr().then(this.finalize);

			}.bind(this));
		},

		fetchMetadata : function (tests) {
		...
		crawlFilesForTests : function (metadata) {
			var tests = config.tests.concat(config.customTests),
